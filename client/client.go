...
func extractUser(data string) (user User, err error) {
	pack := strings.Split(data, ":")
	if len(pack) == 3 {
		user = User{pack[0], pack[1], pack[2]}
	} else {
...
func parseServerConfig(config *ss.Config) {
	...
	if len(config.ServerPassword) == 0 {
		...
		user, err := extractUser(config.Password)
		if err != nil {
			log.Fatal("Failed generating ciphers:", err)
		}

		server_cipher, err := ss.NewCipher(method, user.PublicPassword)
		if err != nil {
			log.Fatal("Failed generating ciphers:", err)
		}

		user_cipher, err := ss.NewCipher(method, user.PrivatePassword)
		if err != nil {
			log.Fatal("Failed generating ciphers:", err)
		}
		...
		for i, s := range srvArr {
			if hasPort(s) {
				...
				servers.srvCipher[i] = &ServerCipher{s, user_cipher, server_cipher, user}
			} else {
				servers.srvCipher[i] = &ServerCipher{net.JoinHostPort(s, srvPort), user_cipher, server_cipher, user}
			}
		}
	} else {
		// multiple servers
		...
		user_cipherCache := make(map[string]*ss.Cipher)
		server_cipherCache := make(map[string]*ss.Cipher)
		i := 0
		for _, serverInfo := range config.ServerPassword {
			...
			user, err := extractUser(passwd)
			if err != nil {
				log.Fatal("Failed generating ciphers:", err)
			}

			user_cipher, ok := user_cipherCache[user.PrivatePassword]
			if !ok {
				var err error
...
func DialWithRawAddr(rawaddr []byte, user User, server string, user_cipher *ss.Cipher, server_cipher *ss.Cipher) (remote *ss.Conn, err error) {
	...
	remote = ss.NewConn(conn, user_cipher)
...
func connectToServer(serverId int, rawaddr []byte, addr string) (remote *ss.Conn, err error) {
	...
	remote, err = DialWithRawAddr(rawaddr, se.user, se.server, se.user_cipher.Copy(), se.server_cipher.Copy())
	if err != nil {
