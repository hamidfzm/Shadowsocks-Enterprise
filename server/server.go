...
func run(port, password string, auth bool) {
	...
	var server_cipher *ss.Cipher
	var user_cipher *ss.Cipher
	...
	for {
		...
		if server_cipher == nil {
			log.Println("creating cipher for port:", port)
			server_cipher, err = ss.NewCipher(config.Method, password)
			if err != nil {
		...
		user, err := authenticate(ss.NewConn(conn, server_cipher.Copy()))
		if err != nil {
			log.Printf("Error authenticating: %s %v\n", port, err)
			conn.Close()
			continue
		}


		user_cipher, err = ss.NewCipher(user.Method, user.Password)
		if err != nil {
			log.Printf("Error generating cipher for user: %s %v\n", user.Name, err)
			conn.Close()
			continue
		}
		go handleConnection(ss.NewConn(conn, user_cipher.Copy()), auth)
	}
}
...
func unifyPortPassword(config *ss.Config) (err error) {
	if len(config.PortPassword) == 0 {
		// this handles both nil PortPassword and empty one
